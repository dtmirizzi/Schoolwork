...
public class EchoNode implements Runnable {
    ...
    public EchoNode(int port, InetSocketAddress[] neighbors, boolean Init) throws Exception {
	this.port = port;
	this.neighbors = neighbors;
	this.init = Init;
	if (init == true) {
	    curWave = EchoMain.makeID(port);
	} else {
	    curWave = EchoMain.makeID(29999);
	}
	...
	if (this.init == true) {
		...
		for (int i = 0; i < neighbors.length; i++) {
		    try {
			sendTo(i, new Message(this.curWave, EchoMain.makeID(port)));
		    } catch (Exception ex) {
			ex.printStackTrace();
		    }
		}
		...
		if (this.curWave.equals(message.getId())) {
		 // echo because q=r
			...
			if (leader) {
			    ...
			    sendTo(parIndex, new Message(this.curWave, EchoMain.makeID(port)));
